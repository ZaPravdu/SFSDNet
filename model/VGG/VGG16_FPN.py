...
def _make_inplace_safe(features):
    """将所有 ReLU 替换为 inplace=False，确保 checkpoint.checkpoint 的梯度正确性"""
    result = []
    for layer in features:
        if isinstance(layer, nn.ReLU):
            result.append(nn.ReLU(inplace=False))
        else:
            result.append(layer)
    return result
...
class VGG16FPN_Stride16(nn.Module):
    def __init__(self, pretrained=True):
        ...
        features = _make_inplace_safe(list(vgg.features.children()))
